...
# Pseudobinaries based on simultaneous mixing on all three sites are considered. 
...
df=pd.DataFrame(columns=["Formula_A","Formula_B","Mixing_Elements_on_Mixing_Site1","Mixing_Elements_on_Mixing_Site2","Mixing_Elements_on_Mixing_Site3"])
			...
			if vecX+vecY+vecZ == 18:				#Applying the 18-electron rule constraint to filter down the possible number of end members.
				...
				for i in range(len(EM)):				#Iterating over end-members to find instances with common Y and Z-site elements.  
					if (x[i]!=x[len(EM)-1]) and (y[i]!=y[len(EM)-1]) and (z[i]!=z[len(EM)-1]):
						df = df.append({"Formula_A":EM[i],"Formula_B":EM[len(EM)-1],"Mixing_Elements_on_Mixing_Site1":[x[i],x[len(EM)-1]],"Mixing_Elements_on_Mixing_Site2":[y[i],y[len(EM)-1]],"Mixing_Elements_on_Mixing_Site3":[z[i],z[len(EM)-1]]},ignore_index = True)

df.to_csv('pseudobinary_design_space_TRIPLE.csv',sep = ',', index=False)
